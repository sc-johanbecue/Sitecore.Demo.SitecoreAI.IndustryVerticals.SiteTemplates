...
export const BackgroundImageBannerStyle = (props: PromoProps): JSX.Element => {
  const id = props.params.RenderingIdentifier;
  const ctaText = props.fields.PromoMoreInfo?.value?.text || 'Learn more';
  const styles = props.params.styles || '';
  const isWhiteFontColor = styles.includes(PromoFlags.WhiteFontColor);
  const isBlackFontColor = styles.includes(PromoFlags.BlackFontColor);

  const textColorClass = isWhiteFontColor
    ? 'text-white'
    : isBlackFontColor
      ? 'text-black'
      : 'text-[#2f2f2f]';
  const subtitleColorClass = isWhiteFontColor
    ? 'text-white/85'
    : isBlackFontColor
      ? 'text-black/80'
  ...
  return (
    <section
      className={`${props.params.styles} relative overflow-hidden py-14 md:py-20`}
      id={id ? id : undefined}
    >
      ...
      <div className="relative z-10 container">
        <div className={clsx('max-w-6xl space-y-6', textColorClass)}>
          <h2 className="text-3xl leading-tight font-medium md:text-4xl">
            <Text field={props.fields.PromoTitle} />
          </h2>

          <div className="text-base leading-relaxed md:text-xl">
            <ContentSdkRichText field={props.fields.PromoDescription} />
          </div>

          <div className={clsx('text-base italic md:text-xl', subtitleColorClass)}>
            <Text field={props.fields.PromoSubTitle} />
          </div>
          ...
          <Link
            ...
            className={clsx(
              'group inline-flex items-center gap-3 font-medium transition-opacity hover:opacity-80',
              textColorClass
            )}
          >
